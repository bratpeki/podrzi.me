...
const dummyNotifications = [
  { id: 1, message: "Zahtjev za kolaboraciju od Marka", type: "collab",seen:false },
  { id: 2, message: "Akcija 'Donacija' dostigla cilj!", type: "goal" ,seen:false },
...
export default function NotificationDropdown() {
  ...
  return (
   <Menu as="div" className="relative inline-block text-left">
     <Menu.Button onClick={markAllAsSeen} className="flex items-center space-x-2 text-white hover:underline">
      ...
      <Menu.Items className="absolute right-0 mt-2 w-72 bg-white divide-y divide-gray-200 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50 text-black">
        {dummyNotifications.map((notification) => (
          <Menu.Item key={notification.id}>
            {({ active }) => (
              <div className={`px-4 py-2 text-sm ${active ? 'bg-gray-100' : ''}`}>
